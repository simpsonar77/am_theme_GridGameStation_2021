...
class UserConfig </ help="Navigation controls: Up/Down (to move up and down) and Page Up/Page Down (to move left and right)" />{
	...
	</ label="Number Rows", help="Select Number of rows", options="1, 2, 3, 4", order=5 /> numRows="3";
        </ label="Number Cols", help="Select Number of cols", options="1, 2, 3, 4, 5", order=6 /> numCols="3";
...
if ( my_config["screen"] == "widescreen" ){
	...
	if ( my_config["style"] == "vertical rectangle" ){
		rows = my_config["numRows"].tointeger();
		cols = my_config["numCols"].tointeger();
